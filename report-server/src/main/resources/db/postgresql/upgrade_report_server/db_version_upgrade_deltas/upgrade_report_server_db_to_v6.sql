...
CREATE TABLE reporting.role_authority(
	...
	CONSTRAINT role_authority_pkey PRIMARY KEY (role_authority_id),
	CONSTRAINT uc_roleauthority_role_authority UNIQUE (role_id,authority_id)
);


-- object: fk_roleauthority_authority | type: CONSTRAINT --
-- ALTER TABLE reporting.role_authority DROP CONSTRAINT IF EXISTS fk_roleauthority_authority CASCADE;
ALTER TABLE reporting.role_authority ADD CONSTRAINT fk_roleauthority_authority FOREIGN KEY (authority_id)
REFERENCES reporting.authority (authority_id) MATCH SIMPLE
ON DELETE NO ACTION ON UPDATE NO ACTION;
-- ddl-end --

-- object: fk_roleauthority_role | type: CONSTRAINT --
-- ALTER TABLE reporting.role_authority DROP CONSTRAINT IF EXISTS fk_roleauthority_role CASCADE;
ALTER TABLE reporting.role_authority ADD CONSTRAINT fk_roleauthority_role FOREIGN KEY (role_id)
...
--
-- Make existing user "user4" a child of "reportadmin", effectively making "user4" an administrator:
...
--
-- Give the "ReST Administrator" role most authorities by creating "role_authority" rows:

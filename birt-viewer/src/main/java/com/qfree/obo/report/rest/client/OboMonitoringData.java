...
public class OboMonitoringData {
	 ...
	 *  TODO Make this method generic:  T -> JsonObject or JsonArray or ... ?
	 */
	public Object getRestData(URL restURI) {	//TODO Use generics here?
		...
		if (cachedData == null || cachedData.getValidAt() < now - OBO_FLOW_REST_CACHE_VALID_MILLIS) {
			synchronized (cache) {
				...
				if (cachedData == null || cachedData.getValidAt() < now - OBO_FLOW_REST_CACHE_VALID_MILLIS) {
					...
					//					RestResult restResult = getRestResource<JsonObject>(restURI);
					//					// Test httpStatus and proceed appropriately?
					//					int httpStatus = restResult.getStatus();
					//					    Object oboFlowManagerJsonObject = restResult.getEntity();
					//			or?		JsonObject oboFlowManagerJsonObject = restResult.getEntity();

					// RestResult -> (<T> httpResponseEntity, int httpStatus)
					JsonObject oboFlowManagerJsonObject = getRestResource(restURI);	//TODO Use generics here?
					 ...
					 * as well.
					 */
					// OboFlowManager oboFlowManager = parseJson(oboFlowManagerJsonObject, restURI);
					OboFlowManager oboFlowManager = parseJson(oboFlowManagerJsonObject);	//TODO Use generics here?
	 ...
	 * @return
	 */
	private OboFlowManager parseJson(JsonObject oboFlowManagerJsonObject) {	//TODO Use generics here?

		/*
		 * TODO Pass "URL restURI" as a parameter and then use a "switch" or "if" block
		 * to select the proper parsing for that URI? We will need to use generics to 
		 * return the appropriate object type - is this possible? Maybe not.
		 */
	...
	private JsonObject getRestResource(URL restURI) {
		...
		//TODO Must treat HTTP status codes other than 200 (OK) as well as unexpected exceptions.
		...
		return oboFlowManagerJsonObject;
	}
